...
export function ProductGrid({ categories, products, cartProductIds, onSelectProduct, disabled = false }: ProductGridProps) {
  ...
  const visibleProducts = products.filter(
    (product) => (selectedCategoryId === 'all' || product.categoryId === selectedCategoryId)
      && product.name.toLocaleLowerCase().includes(query.trim().toLocaleLowerCase()),
  )
    ...
    <div className="flex min-w-0 flex-col gap-4">
      ...
      {visibleProducts.length === 0 ? (
        <p className="p-4 text-center text-slate-500">
          {products.length === 0 ? 'No hay productos disponibles.' : 'No hay productos que coincidan con la búsqueda y categoría.'}
        </p>
      ) : (
